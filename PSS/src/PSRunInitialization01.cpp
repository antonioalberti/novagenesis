/*
	...
	Name:		PSRunInitialization01
	Object:		PSRunInitialization01
	File:		PSRunInitialization01.cpp
...
PSRunInitialization01::PSRunInitialization01 (string _LN, Block *_PB, MessageBuilder *_PMB) : Action (_LN, _PB, _PMB)
{
}

PSRunInitialization01::~PSRunInitialization01 ()
{
}
...
int
PSRunInitialization01::Run (Message *_ReceivedMessage, CommandLine *_PCL, vector<Message *> &ScheduledMessages, Message *&InlineResponseMessage)
{
  int Status = OK;
  ...
  PB->PP->NewMessage (GetTime () + PPS->DelayBeforeRunInitiatilization, 0, false, StoringInitialBinds);
  ...
  PPS->PGW->PushToInputQueue (StoringInitialBinds);
  ...
  PB->S << Offset << "(Loading customized parameters (if available) at " << PB->GetPath () << "PSS.ini)" << endl;

  File F3;

  F3.OpenInputFile ("PSS.ini", PB->GetPath (), "DEFAULT");
	  ...
	  if (Temp > 0)
		{
		  if (Parameter == "DelayBeforeRunInitiatilization")
			{
			  PPS->DelayBeforeRunInitiatilization = Temp;
		  ...
		  if (Parameter == "DelayBeforeGIRSDiscovery")
			{
			  PPS->DelayBeforeGIRSDiscovery = Temp;
		  ...
		  if (Parameter == "DelayBeforePIDPublishing")
			{
			  PPS->DelayBeforePIDPublishing = Temp;
		  ...
		  if (Parameter == "DelayBeforeRunExposition")
			{
			  ...
			  PPS->DelayBeforeRunExposition = Temp;
		  ...
		  if (Parameter == "DelayBeforeSendingToGIRS")
			{
			  PPS->DelayBeforeSendingToGIRS = Temp;
		  ...
		  if (Parameter == "DelayBeforeSendingANotification")
			{
			  PPS->DelayBeforeSendingANotification = Temp;
		  ...
		  if (Parameter == "DelayBeforeRunPeriodic")
			{
			  PPS->DelayBeforeRunPeriodic = Temp;
