...
int PSPubNotify01::Run(Message* _ReceivedMessage, CommandLine* _PCL, vector<Message*>& ScheduledMessages, Message*& InlineResponseMessage)
{
  int Status = OK;
  string Offset = "                    ";
  unsigned int NA = 0;
            ...
            for (unsigned int i = 3; i < NA; i++)
            {
              ...
              if (Notification.size() == 5)
              {
                ...
                if (Type == "pub")
                {
                  ...
                  PMB->NewPubNotificationCommandLine("0.1", PB->StringToInt(Category.at(0)), &Key, &PPS->Publisher, Notify, PCL);
