...
int PSSubBind01::Run(Message* _ReceivedMessage, CommandLine* _PCL, vector<Message*>& ScheduledMessages, Message*& InlineResponseMessage)
{
  int Status = OK;
  string Offset = "                    ";
  unsigned int NA = 0;
      ...
      if (_PCL->GetArgument(0, Category) == OK && _PCL->GetArgument(1, Key) == OK)
      {
        if (Category.size() > 0 && Key.size() > 0)
        {
          ...
          for (unsigned int i = 0; i < Key.size(); i++)
          {
            ...
            // Add routing: -m --cl with destination = HT (same as PSMsgCl01)
