...
PSPubBind01::PSPubBind01(string _LN, Block* _PB, MessageBuilder* _PMB)
    : Action(_LN, _PB, _PMB)
{
}

PSPubBind01::~PSPubBind01()
{
}

// Run the actions behind a received command line
// ng -p --b _Version [ < 1 string _Category > < 1 string _Key > < _ValuesSize string S_1 ... S_ValuesSize > ]
int PSPubBind01::Run(Message* _ReceivedMessage, CommandLine* _PCL, vector<Message*>& ScheduledMessages, Message*& InlineResponseMessage)
{
  int Status = OK;
  string Offset = "                    ";
  unsigned int NA = 0;
      ...
      if (_PCL->GetArgument(0, Category) == OK && _PCL->GetArgument(1, Key) == OK && _PCL->GetArgument(2, Values) == OK)
      {
        if (Category.size() > 0 && Key.size() > 0 && Values.size() > 0)
        {
          if (PB->State == "operational")
          {
            ...
            PMB->NewCommonCommandLine("-sr", "--b", "0.2", PB->StringToInt(Category.at(0)), Key.at(0), &Values, InlineResponseMessage, PCL);
          }
        }
        else
        {
