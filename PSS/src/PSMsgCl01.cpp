...
PSMsgCl01::PSMsgCl01(string _LN, Block* _PB, MessageBuilder* _PMB)
    : Action(_LN, _PB, _PMB)
{
}
...
int PSMsgCl01::Run(Message* _ReceivedMessage, CommandLine* _PCL, vector<Message*>& ScheduledMessages, Message*& InlineResponseMessage)
{
  int Status = ERROR;
  unsigned int NA = 0;
          ...
          if (Destinations.size() == 4)
          {
            if (PB->State == "operational")
            {
              ...
              if (PPS->GIRSTuples.size() > 0)
              {
                ...
                if (PMB->NewConnectionLessCommandLine("0.2", &Limiters, &Sources, &Destinations, InlineResponseMessage, PCL) == OK)
                {
                  Status = OK;
                }
                else
