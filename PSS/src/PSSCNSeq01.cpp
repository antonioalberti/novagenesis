...
PSSCNSeq01::PSSCNSeq01 (string _LN, Block *_PB, MessageBuilder *_PMB) : Action (_LN, _PB, _PMB)
{
}

PSSCNSeq01::~PSSCNSeq01 ()
{
}
...
int
PSSCNSeq01::Run (Message *_ReceivedMessage, CommandLine *_PCL, vector<Message *> &ScheduledMessages, Message *&InlineResponseMessage)
{
  int Status = ERROR;
  string ReceivedSCN;
  string NewSCN;
		  ...
		  if (ReceivedSCN != "")
			{
			  if (PB->State == "operational")
				{
				  PMB->NewSCNCommandLine ("0.2", ReceivedSCN, InlineResponseMessage, PCL);
				}
