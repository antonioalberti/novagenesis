...
CoreRunInitialize01::Run (Message *_ReceivedMessage, CommandLine *_PCL, vector<Message *> &ScheduledMessages, Message *&InlineResponseMessage)
{
  ...
  while (F3.getline (Line, sizeof (Line), '\n'))
	{
	  ...
	  if (Parameter == "NumberOfPublications")
		{
		  Temp = PB->StringToDouble (Value);

		  PCore->NumberOfPublications = Temp;

		  PB->S << Offset << "(NumberOfPublications is " << Temp << ")" << endl;
		}

	  if (Parameter == "NumberOfSubscriptions")
		{
		  Temp = PB->StringToDouble (Value);

		  PCore->NumberOfSubscriptions = Temp;

		  PB->S << Offset << "(NumberOfSubscriptions is " << Temp << ")" << endl;
		}

	  if (Parameter == "NumberOfMessagesPerBurst")
		{
		  Temp = PB->StringToDouble (Value);

		  PCore->NumberOfMessagesPerBurst = Temp;

		  PB->S << Offset << "(NumberOfMessagesPerBurst is " << Temp << ")" << endl;
		}

	  if (Parameter == "NumberOfPubsPerMessage")
		{
		  Temp = PB->StringToDouble (Value);

		  PCore->NumberOfPubsPerMessage = Temp;

		  PB->S << Offset << "(NumberOfPubsPerMessage is " << Temp << ")" << endl;
		}

	  if (Parameter == "NumberOfHTSs")
		{
		  PCore->NumberOfHTSs = Value;

		  PB->S << Offset << "(NumberOfHTSs is " << Value << ")" << endl;
		}
	  ...
	  if (Parameter == "Trial")
		{
		  ...
		  PB->S << Offset << "(Trial is " << Value << ")" << endl;
  ...
  // Statistic variables
  PCore->pubrtt = new OutputVariable (PCore);
  PCore->subrtt = new OutputVariable (PCore);
  //tsmiup1=new OutputVariable(this);

  string H1 =
	  "pubrtt_" + PCore->DoubleToString (PCore->NumberOfPublications) + "_" + PCore->NumberOfHTSs + "_" + PCore->Trial;
  string H2 =
	  "subrtt_" + PCore->DoubleToString (PCore->NumberOfPublications) + "_" + PCore->NumberOfHTSs + "_" + PCore->Trial;

  PB->S << Offset << "(H1 = " << H1 << ")" << endl;
  PB->S << Offset << "(H2 = " << H2 << ")" << endl;

  PCore->pubrtt
	  ->Initialization (H1, "MEAN_ARITHMETIC", "Binding publishing round trip time (seconds)", 1); // Sync reference is the time at the machine where the App process is running
  PCore->subrtt->Initialization (H2, "MEAN_ARITHMETIC", "Binding subscribing round trip time (seconds)", 1);
