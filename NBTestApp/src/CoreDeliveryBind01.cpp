...
CoreDeliveryBind01::CoreDeliveryBind01(string _LN, Block* _PB, MessageBuilder* _PMB)
    : Action(_LN, _PB, _PMB)
{
}
          ...
          if (PS->Status == "Waiting delivery")
          {
            ...
            for (unsigned int t = 0; t < Key.size(); t++)
            {
              if (Key.at(t) == PS->Key)
              {
                ...
                PB->S << Offset << setprecision(10) << "(The entire subscription time was  " << DeltaT
                      << " seconds)" << endl;
                ...
                // PB->S << Offset <<  "(Deleting the subscription with index = "<<i<<")" << endl;
