...
HTMsgCl01::HTMsgCl01(string _LN, Block* _PB, MessageBuilder* _PMB)
    : Action(_LN, _PB, _PMB)
{
}
...
int HTMsgCl01::Run(Message* _ReceivedMessage, CommandLine* _PCL, vector<Message*>& ScheduledMessages, Message*& InlineResponseMessage)
{
  int Status = ERROR;
  ...
  if (_PCL->GetNumberofArguments(NA) == OK)
  {
    ...
    if (NA == 3)
    {
      ...
      if (Limiters.size() > 0 && Sources.size() > 0 && Destinations.size() > 0)
      {
        ...
        if (PMB->NewConnectionLessCommandLine(_PCL->Version, &Limiters, &Destinations, &Sources, InlineResponseMessage, NewHTMsgCl01) == OK)
        {
