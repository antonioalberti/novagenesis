...
GWHelloIPC01::GWHelloIPC01(string _LN, Block* _PB, MessageBuilder* _PMB)
    : Action(_LN, _PB, _PMB)
{
}
...
int GWHelloIPC01::Run(Message* _ReceivedMessage, CommandLine* _PCL, vector<Message*>& ScheduledMessages, Message*& InlineResponseMessage)
{
  int Status = OK;
  unsigned int NA = 0;
...
#ifdef DEBUG
		  PB->S << Offset <<  "(Scheduled the following message to the peer "<<PeerData.at(1)<<")" << endl;
