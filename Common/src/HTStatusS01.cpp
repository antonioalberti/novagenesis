...
HTStatusS01::HTStatusS01(string _LN, Block* _PB, MessageBuilder* _PMB)
    : Action(_LN, _PB, _PMB)
{
}
...
int HTStatusS01::Run(Message* _ReceivedMessage, CommandLine* _PCL, vector<Message*>& ScheduledMessages, Message*& InlineResponseMessage)
{
  int Status = ERROR;
  unsigned int NA = 0;
  ...
  if (_PCL->GetNumberofArguments(NA) == OK)
  {
    ...
    if (NA == 2)
    {
      ...
      {
        if (RelatedCommand.at(0) == "-sr" && RelatedCommand.at(1) == "--b" && StatusCode.at(0) == "0" && PB->State == "operational")
        {
