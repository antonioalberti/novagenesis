					...
					if (Category.size() > 0 && Key.size() > 0 && Values.size() > 0) // VVV
						{
							if (PCore->GenerateStoreBindingsMsgCl01 == true)
								{
									// ****************************************************
									// Generate the ng -msg --cl header for App::HT block
									// ****************************************************
							...
							// Generate the ng -store --bind for the local HT
							PMB->NewCommonCommandLine("-store","--bind","0.1",PB->StringToInt(Category.at(0)),Key.at(0),&Values,InlineResponseMessage,PCL);
