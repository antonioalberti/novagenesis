...
void CoreRunPublish02::CreatePublishMessage(string _FileName, vector<Tuple*> &_PubNotify, vector<Tuple*> &_SubNotify)
{
	...
	PB->PP->NewMessage(GetTime(),1,true,"Not_Necessary",_FileName,"Not_Necessary",NGFILES_PATH,Publish);
	...
	if (PayloadSize > 0)
		{
			...
			PCore->GetFileContentHash(_FileName,PayloadHash,NGFILES_PATH);
			...
			// Set file to be published
			Values.push_back(PayloadHash);

			// Category: 18
			// Hash(Content) => Content

			// First publish is a PayloadHash
			PB->S << endl << Offset <<"(Publishing-name-content [" << _FileName << "] => ["<< PayloadHash <<"] )"<< endl;

			//PMB->NewCommonCommandLine("-pub","--bind","0.1",1,PayloadHash,&Values,Publish,PCL);
			PMB->NewCommonCommandLine("-pub","--bind","0.1",18,_FileName,&Values,Publish,PCL);

			Values.clear();

			// Category: 2
			// Content [ Wordlist, .. ]	=> Hash(Content)

			Values.push_back(_FileName);

			NGHelperFiles ngh(NGFILES_PATH,_FileName);

			PB->S << endl << Offset <<"(Publishing-name2name [" << _FileName << "] => ["<< _FileName <<"] )"<< endl;
			PMB->NewCommonCommandLine("-pub","--bind","0.1",2,_FileName,&Values,Publish,PCL);

// for each ( keywords )

			while(ngh.hasWord()){
				PB->S << endl << Offset <<"(Publishing [" << ngh.getWord() << "] => ["<< _FileName <<"] )"<< endl;
				//PMB->NewCommonCommandLine("-pub","--bind","0.1",1,ngh.getWord(),&Values,Publish,PCL);
				PMB->NewCommonCommandLine("-pub","--bind","0.1",2,ngh.getWord(),&Values,Publish,PCL);
			}
