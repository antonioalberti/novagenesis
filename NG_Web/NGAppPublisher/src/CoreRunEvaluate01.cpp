					...
					if (ScheduledMessages.size() > 0 )
						{
							...
							NGFilesSubscribed* ngf = NGFilesSubscribed::getInstance(NGFILES_PATH);
							...
							if( Run != 0 && ngf->next() ){
								...
								//PS->Status = "Delete";
#endif
								PB->S << Offset1 << "(Publishing 0.2 : Scheduled.)"<<endl;
								...
								PCL->SetArgumentElement(0,0,ngf->getFile());

								ClearScheduledMessage=false;

								PB->S << Offset1 << "(File:"<< ngf->getFile() <<")"<<endl;

								//Status=OK;
								// Enable publication of all the photos again to notify the new peer
								//PCore->PublishAllPhotos = true;
							} else {
								PB->S << Offset1 << "(HTML was published)"<<endl;
