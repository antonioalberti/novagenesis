...
int checkDestination(NgEPGS** ngEPGS, NgNetInfo destPeer)
{
  ...
  {
    if (ng_strcmp(destPeer.HID, my->HID) == 0 || ng_strcmp(destPeer.HID, "FFFFFFFF") == 0)
    {
    ...
    else
    {
      if (peersCount > 0)
      {
        ...
        {
          if (ng_strcmp(peers[i]->HID, destPeer.HID) == 0 && ng_strcmp(peers[i]->OSID, destPeer.OSID) == 0 && ng_strcmp(peers[i]->PID, destPeer.PID) == 0 && ng_strcmp(peers[i]->BID, destPeer.BID) == 0)
          {
...
int addPeer(NgEPGS** ngEPGS, NgNetInfo peer)
{
  ...
  for (i = 0; i < (*ngEPGS)->PeersNetInfoCount; i++)
  {
    if (ng_strcmp((*ngEPGS)->PeersNetInfo[i]->LIMITER, peer.LIMITER) == 0)
    {
      ...
      {
        if (ng_strcmp((*ngEPGS)->PeersNetInfo[i]->OSID, peer.OSID) == 0)
        {
          if (ng_strcmp((*ngEPGS)->PeersNetInfo[i]->PID, peer.PID) == 0)
          {
            if (ng_strcmp((*ngEPGS)->PeersNetInfo[i]->BID, peer.BID) == 0)
            {
              ng_printf("\nPeer already exists!\n");
              return NG_OK;
            }
...
int RunHello(NgEPGS* ngEPGS)
{
  ...
  if (ng_strcmp(ngEPGS->MyNetInfo->Stack, NG_STACK_ETHERNET) == 0 || ng_strcmp(ngEPGS->MyNetInfo->Stack, NG_STACK_WIFI) == 0)
  {
