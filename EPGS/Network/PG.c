...
int newMessageReceived(struct _ng_epgs** ngEPGS, const char* message, int rcvdMsgSize)
{
  ...
  {
    bufferIndex = DEFAULT_MTU - (HEADER_SEGMENTATION_FIELD_SIZE + ETHERNET_MAC_ADDR_FIELD_SIZE + ETHERNET_MAC_ADDR_FIELD_SIZE + ETHERNET_TYPE_FIELD_SIZE + HEADER_SIZE_FIELD_SIZE) +
                  (DEFAULT_MTU - (HEADER_SEGMENTATION_FIELD_SIZE + ETHERNET_MAC_ADDR_FIELD_SIZE + ETHERNET_MAC_ADDR_FIELD_SIZE + ETHERNET_TYPE_FIELD_SIZE)) * (msgNumber - 1);
  }
