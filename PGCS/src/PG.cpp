...
int PG::SendToARawSocket(string _Interface, string _Identifier, unsigned int _Size, Message* M)
{
  ...
  if (PP->GetHTBindingValues(17, _Identifier, SIDs) == OK)
  {
    if (SIDs->size() == 1)
    {
      ...
      S << endl
        << endl
        << "[7]                 (Sending the message number " << MessageNumber
      ...
      if (ioctl(StringToInt(SIDs->at(0)), SIOCGIFINDEX, &buffer) >= 0)
      {
        ...
        S << endl
          << Offset << "(The interface index is = " << ifindex << ")" << endl;
        ...
        if (M->GetMessageSize(MessageSize) == OK)
        {
          if (MessageSize > 0)
          {
            ...
            if (NoCL > 2)
            {
              ...
              MessageNumber = static_cast<unsigned int>(rand() % 4294967289) + 1; // It never will be zero, therefore zero is problem
              ...
              for (unsigned int i = 0; i < NoS; i++)
              {
                ...
                if (i < (NoS - 1))
                {
                  ...
                  HeaderSegmentationField[0] = static_cast<unsigned char>((MessageNumber >> (8 * 3)) & 0xff);
                  HeaderSegmentationField[1] = static_cast<unsigned char>((MessageNumber >> (8 * 2)) & 0xff);
                  HeaderSegmentationField[2] = static_cast<unsigned char>((MessageNumber >> (8 * 1)) & 0xff);
                  HeaderSegmentationField[3] = static_cast<unsigned char>((MessageNumber >> (8 * 0)) & 0xff);
                  HeaderSegmentationField[4] = static_cast<unsigned char>((SequenceNumber >> (8 * 3)) & 0xff);
                  HeaderSegmentationField[5] = static_cast<unsigned char>((SequenceNumber >> (8 * 2)) & 0xff);
                  HeaderSegmentationField[6] = static_cast<unsigned char>((SequenceNumber >> (8 * 1)) & 0xff);
                  HeaderSegmentationField[7] = static_cast<unsigned char>((SequenceNumber >> (8 * 0)) & 0xff);
                ...
                while (Tentatives > 0)
                {
                  ...
                  numbytes = static_cast<int>(sendto(StringToInt(SIDs->at(0)), (void*)frame.buffer, frame_len, 0, (struct sockaddr*)&saddrll, sizeof(saddrll)));
...
void PG::SocketDispatcher3()
{
  ...
  F1 << endl
     << Offset << "(Starting running the Ethernet or Wi-Fi socket dispatcher thread)" << endl;
  ...
  while (1)
  {
    for (unsigned int x = 0; x < PPGCS->SSIDs->size(); x++)
    {
      if (PPGCS->Sizes->size() > 0)
      {
        ...
        F1 << endl
           << endl
           << Offset << "(The socket " << x << " for stack " << Stack << " has block size = " << BlockSize << ")" << endl;
#endif
        ...
        if (Stack == "Ethernet" || Stack == "Wi-Fi") // Ethernet or Wi-Fi socket
        {
          if (PPGCS->MyMACAddress != "") // The Ethernet case
          {
...
#ifdef DEBUG3
            F1 << endl
               << Offset << "(The server socket ID (SSID) is " << SSID << ")" << endl;
#endif
            ...
            F1 << endl
               << Offset << "(I do received " << receivedbytes << " in this socket at instant " << setprecision(12) << GetTime() << ")" << endl;
#endif

            if (receivedbytes > 0 && saddrll.sll_protocol == 13330) // It is required to check the type of protocol at link layer. It is equivalent to Eth.type 00x1234
            {
...
#ifdef DEBUG6
                    F1 << endl
                       << Offset << "(Transferring the message " << PMR3->MessageNumber
                    ...
                    while (1)
                    {
                      ...
                      else
                      {
                        ...
                        if (sem_trywait(mutex[v]) == 0)
                        {
                          ...
                          if (sem_post(mutex[v]) != 0) // Successfully locked the semaphore, so unlock it
                          {
...
void PG::FinishReceivingThread(unsigned int Index)
{
  ...
  F1 << endl
     << Offset << "(Starting running the thread " << Index << " to receive Ethernet or Wi-Fi frames)" << endl;
  ...
  while (1)
  {
    ...
    if (sem_trywait(cached_mutex) == 0)
    {
      ...
      if (VS > 0)
      {
        ...
        F1 << endl
           << endl
           << Offset << "(Copied frames to temporary vector)" << endl;
#endif
      }
...
#ifdef DEBUG3
      F1 << endl
         << Offset << "(--- Delivering message " << y << " to the SMH ---)" << endl;
#endif
...
void PG::ReceiveFromAUDPSocket()
{
  ...
  F1 << endl
     << Offset << "(Starting running the thread to receive UDP segments)" << endl;
  ...
  while (1)
  {
    for (unsigned int x = 0; x < PPGCS->SSIDs->size(); x++)
    {
      ...
      if (numbytes > 0)
      {
        ...
        F1 << endl
           << endl
           << "[7]                 (I do received a packet with " << numbytes
        ...
        if (MN >= 0 && SN >= 0)
        {
          ...
          { // Continue receiving a message!!
            ...
            F1 << endl
               << endl
               << "[9]                 (Continue receiving a message with number " << MN
               << " at socket " << SSID << ")" << endl;
            ...
            // Stop criteria
            if (PMR2->ReceivedSoFar == PMR2->MessageSize && PMR2->SegmentsSoFar == PMR2->NoS)
            {
