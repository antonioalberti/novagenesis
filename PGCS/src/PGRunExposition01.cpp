...
int PGRunExposition01::Run(Message* _ReceivedMessage, CommandLine* _PCL, vector<Message*>& ScheduledMessages, Message*& InlineResponseMessage)
{
  int Status = OK;
  string Offset = "                    ";
  CommandLine* PCL;
  ...
  if (PB->StopProcessingMessage == false)
  {
    ...
    if (PPGCS->Stacks != NULL && PPGCS->Stacks->size() > 0)
    {
      ...
      for (unsigned int i = 0; i < PPG->PGCSTuples.size(); i++)
      {
        ...
        if (PB->PP->DiscoverHomonymsBlocksBIDsFromPID(PPG->PGCSTuples[i]->Values[2], "HT", HTs, PB) == OK)
        {
          if (HTs->size() == 1)
          {
            // *****************************************************************************
            ...
            if (PB->PP
                    ->DiscoverHomonymsEntitiesTuplesFromProcessAndBlockLegibleNames("NRNCS", "NR", NRNCSs, PB) == OK)
            {
                ...
                (NRNCS_OK == true && NRNCS_ERROR == false))
            {
              ...
              for (unsigned int f = 0; f < HTSs.size(); f++)
              {
                ...
                PMB->NewStoreBindingCommandLineFromHashLNToSCN("0.1", 9, "Host", PTuple->Values.at(0), Exposition, PCL);

                PMB->NewStoreBindingCommandLineFromHashLNToSCN("0.1", 2, "OS", PTuple->Values.at(1), Exposition, PCL);

                PMB->NewStoreBindingCommandLineFromSCNToSCN("0.1", 6, PTuple->Values.at(0), PTuple->Values.at(1), Exposition, PCL);

                PMB->NewStoreBindingCommandLineFromSCNToSCN("0.1", 5, PTuple->Values.at(1), PTuple->Values.at(2), Exposition, PCL);

                PMB->NewStoreBindingCommandLineFromSCNToSCN("0.1", 5, PTuple->Values.at(2), PTuple->Values.at(3), Exposition, PCL);

                PMB->NewStoreBindingCommandLineFromHashLNToSCN("0.1", 2, "HTS", PTuple->Values.at(2), Exposition, PCL);

                PMB->NewStoreBindingCommandLineSCNToHashLN("0.1", 3, PTuple->Values.at(2), "HTS", Exposition, PCL);

                PMB->NewStoreBindingCommandLineFromHashLNToSCN("0.1", 2, "DHT", PTuple->Values.at(3), Exposition, PCL);

                PMB->NewStoreBindingCommandLineSCNToHashLN("0.1", 3, PTuple->Values.at(3), "DHT", Exposition, PCL);
              }
              ...
              for (unsigned int g = 0; g < GIRSs.size(); g++)
              {
                ...
                PMB->NewStoreBindingCommandLineFromHashLNToSCN("0.1", 2, "GIRS", PTuple->Values.at(2), Exposition, PCL);

                PMB->NewStoreBindingCommandLineSCNToHashLN("0.1", 3, PTuple->Values.at(2), "GIRS", Exposition, PCL);

                PMB->NewStoreBindingCommandLineFromHashLNToSCN("0.1", 2, "IR", PTuple->Values.at(3), Exposition, PCL);

                PMB->NewStoreBindingCommandLineSCNToHashLN("0.1", 3, PTuple->Values.at(3), "IR", Exposition, PCL);
              }
              ...
              for (unsigned int h = 0; h < PSSs.size(); h++)
              {
                ...
                PMB->NewStoreBindingCommandLineFromHashLNToSCN("0.1", 2, "PSS", PTuple->Values.at(2), Exposition, PCL);

                PMB->NewStoreBindingCommandLineSCNToHashLN("0.1", 3, PTuple->Values.at(2), "PSS", Exposition, PCL);

                PMB->NewStoreBindingCommandLineFromHashLNToSCN("0.1", 2, "PS", PTuple->Values.at(3), Exposition, PCL);

                PMB->NewStoreBindingCommandLineSCNToHashLN("0.1", 3, PTuple->Values.at(3), "PS", Exposition, PCL);
              }
              ...
              for (unsigned int h = 0; h < NRNCSs.size(); h++)
              {
                ...
                PMB->NewStoreBindingCommandLineFromHashLNToSCN("0.1", 2, "NRNCS", PTuple->Values.at(2), Exposition, PCL);

                PMB->NewStoreBindingCommandLineSCNToHashLN("0.1", 3, PTuple->Values.at(2), "NRNCS", Exposition, PCL);

                PMB->NewStoreBindingCommandLineFromHashLNToSCN("0.1", 2, "NR", PTuple->Values.at(3), Exposition, PCL);

                PMB->NewStoreBindingCommandLineSCNToHashLN("0.1", 3, PTuple->Values.at(3), "NR", Exposition, PCL);
              }
