...
PGHelloIHC02::PGHelloIHC02(string _LN, Block* _PB, MessageBuilder* _PMB)
    : Action(_LN, _PB, _PMB)
{
}
...
int PGHelloIHC02::Run(Message* _ReceivedMessage, CommandLine* _PCL, vector<Message*>& ScheduledMessages, Message*& InlineResponseMessage)
{
  int Status = OK;
  PG* PPGB = 0;
  string HID;
        ...
        if (MyStack == PeerStack)
        {
          ...
          else // TODO: FIXP/Oct2021- This case is required to register EPGS on this PGCS
          {
            ...
            if (StoreNewPGCS2 == true && ReceivedElements.at(0) != PB->PP->GetHostSelfCertifyingName()) // Just one PGCS per host
            {
              PB->S << endl
                    << endl
                    << Offset << "(A new peer EPGS was discovered without previous knowledge: "
                    << ReceivedElements.at(2) << " at the node identified by " << PeerIdentifier << ")"
                    << endl
                    << endl
                    << endl;
...
int PGHelloIHC02::ScheduleStoreBindings(string _Case, vector<string>& _ReceivedElements, string _PeerIdentifier, string _PeerStack)
{
  ...
  PMB->NewStoreBindingCommandLineFromHashLNToSCN("0.1", 9, "Host", _ReceivedElements.at(0), StoreBind01Msg, StoreBind01);
  ...
  PMB->NewStoreBindingCommandLineFromHashLNToSCN("0.1", 2, "OS", _ReceivedElements.at(1), StoreBind01Msg, StoreBind01);
  ...
  PMB->NewStoreBindingCommandLineFromHashLNToSCN("0.1", 2, "PGCS", _ReceivedElements.at(2), StoreBind01Msg, StoreBind01);
  ...
  PMB->NewStoreBindingCommandLineFromHashLNToSCN("0.1", 2, "PG", _ReceivedElements.at(3), StoreBind01Msg, StoreBind01);
  ...
  PMB->NewStoreBindingCommandLineFromHashLNToSCN("0.1", 2, "GW", _ReceivedElements.at(4), StoreBind01Msg, StoreBind01);
  ...
  PMB->NewStoreBindingCommandLineFromHashLNToSCN("0.1", 2, "HT", _ReceivedElements.at(5), StoreBind01Msg, StoreBind01);
  ...
  PMB->NewStoreBindingCommandLineFromHIDToIdentifier("0.1", _ReceivedElements.at(0), _PeerIdentifier, StoreBind01Msg, StoreBind01);

  // ******************************************************
  // Binding Peer PGCS identifier to HID.
  // ******************************************************

  PMB->NewStoreBindingCommandLineFromIdentifierToHID("0.1", _PeerIdentifier, _ReceivedElements.at(0), StoreBind01Msg, StoreBind01);

  // ******************************************************
  // Binding Peer PGCS HID to Hash(Peer PGCS stack).
  // ******************************************************

  PMB->NewStoreBindingCommandLineSCNToHashLN("0.1", 8, _ReceivedElements.at(0), _PeerStack, StoreBind01Msg, StoreBind01);
  ...
  if (_Case == "-de" && PPGCS->CSIDs->size() > 0)
  {
    ...
    PMB->NewStoreBindingCommandLineFromIdentifierToSID("0.1", _PeerIdentifier, PPGCS->CSIDs->at(0), StoreBind01Msg, StoreBind01);
  }
