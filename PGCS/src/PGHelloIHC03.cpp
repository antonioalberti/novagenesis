...
PGHelloIHC03::PGHelloIHC03(string _LN, Block* _PB, MessageBuilder* _PMB)
    : Action(_LN, _PB, _PMB)
{
}
...
int PGHelloIHC03::Run(Message* _ReceivedMessage, CommandLine* _PCL, vector<Message*>& ScheduledMessages, Message*& InlineResponseMessage)
{
  int Status = OK;
  ...
  if (MsgCl01 != 0)
  {
    ...
    for (unsigned int i = 0; i < PPGCS->Stacks->size(); i++)
    {
      ...
      if (MyStack == PeerStack && PeerIdentifier == MyPeerIdentifier)
      {
        ...
        if (StoreNewPGS == true && PeerTuple.at(0) != PB->PP->GetHostSelfCertifyingName()) // Just one PGCS per host
        {
          LOG("(A new peer PGCS was registered via point to point: " << PeerTuple.at(2)
                                                                     << " at the node identified by " << PeerIdentifier << ")");
        ...
        PMB->NewStoreBindingCommandLineFromHashLNToSCN("0.1", 9, "Domain", PeerTuple.at(0), StoreBind01Msg, StoreBind01);

        PMB->NewStoreBindingCommandLineSCNToHashLN("0.1", 8, PeerTuple.at(0), "Domain", StoreBind01Msg, StoreBind01);

        // ******************************************************
        // Binding Received HID to Hash("Host") and vice-versa
        // ******************************************************

        PMB->NewStoreBindingCommandLineFromHashLNToSCN("0.1", 9, "Host", PeerTuple.at(1), StoreBind01Msg, StoreBind01);

        PMB->NewStoreBindingCommandLineSCNToHashLN("0.1", 8, PeerTuple.at(1), "Host", StoreBind01Msg, StoreBind01);

        // ******************************************************
        // Binding Received OSID to Hash("OS") and vice-versa
        // ******************************************************

        PMB->NewStoreBindingCommandLineFromHashLNToSCN("0.1", 2, "OS", PeerTuple.at(2), StoreBind01Msg, StoreBind01);

        PMB->NewStoreBindingCommandLineSCNToHashLN("0.1", 3, PeerTuple.at(2), "OS", StoreBind01Msg, StoreBind01);
        ...
        PMB->NewStoreBindingCommandLineFromHashLNToSCN("0.1", 2, "PGCS", PeerTuple.at(3), StoreBind01Msg, StoreBind01);

        PMB->NewStoreBindingCommandLineSCNToHashLN("0.1", 3, PeerTuple.at(3), "PGCS", StoreBind01Msg, StoreBind01);
        ...
        PMB->NewStoreBindingCommandLineFromHashLNToSCN("0.1", 2, "PG", PeerTuple.at(4), StoreBind01Msg, StoreBind01);

        PMB->NewStoreBindingCommandLineSCNToHashLN("0.1", 3, PeerTuple.at(4), "PG", StoreBind01Msg, StoreBind01);
        ...
        PMB->NewStoreBindingCommandLineFromHashLNToSCN("0.1", 2, "GW", PeerTuple.at(1), StoreBind01Msg, StoreBind01);

        PMB->NewStoreBindingCommandLineSCNToHashLN("0.1", 3, PeerTuple.at(1), "GW", StoreBind01Msg, StoreBind01);
        ...
        PMB->NewStoreBindingCommandLineFromHashLNToSCN("0.1", 2, "HT", PeerNames.at(2), StoreBind01Msg, StoreBind01);

        PMB->NewStoreBindingCommandLineSCNToHashLN("0.1", 3, PeerNames.at(2), "HT", StoreBind01Msg, StoreBind01);
        ...
        PMB->NewStoreBindingCommandLineFromHashLNToSCN("0.1", 2, "Core", PeerNames.at(3), StoreBind01Msg, StoreBind01);

        PMB->NewStoreBindingCommandLineSCNToHashLN("0.1", 3, PeerNames.at(3), "Core", StoreBind01Msg, StoreBind01);

        // ******************************************************
        // Binding Peer PGCS HID to informed Peer Identifier.
        // ******************************************************

        PMB->NewStoreBindingCommandLineFromHIDToIdentifier("0.1", PeerNames.at(1), PeerIdentifier, StoreBind01Msg, StoreBind01);

        // ******************************************************
        // Binding Peer PGCS identifier to HID.
        // ******************************************************

        PMB->NewStoreBindingCommandLineFromIdentifierToHID("0.1", PeerIdentifier, PeerNames.at(1), StoreBind01Msg, StoreBind01);

        // ******************************************************
        // Binding Peer PGCS HID to Hash(Peer PGCS stack).
        // ******************************************************

        PMB->NewStoreBindingCommandLineSCNToHashLN("0.1", 8, PeerNames.at(1), PeerStack, StoreBind01Msg, StoreBind01);
