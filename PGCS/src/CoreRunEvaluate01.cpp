  ...
  if (PCore->PSTuples.size() > 0)
  {
    if (PCore->NextPeerEvaluationTime < Time)
    {
      ...
      if (PB->PP->DiscoverHomonymsEntitiesIDsFromLN(2, "IoT", Candidates, PB) == OK &&
          ...
          PB->PP->DiscoverHomonymsEntitiesTuplesFromProcessAndBlockLegibleNames("IoTTestApp", "Core", Apps, PB) == OK)
      {
...
int CoreRunEvaluate01::CheckSubscriptions(vector<Message*>& _ScheduledMessages, bool _ClearScheduledMessage)
{
  ...
  if (PCore->PSTuples.size() > 0)
  {
    ...
    for (unsigned int i = 0; i < PCore->Subscriptions.size(); i++)
    {
      ...
      PB->S << endl
            << Offset1 << "(The publisher is a " << PS->LN << " and have the index " << Index << ")" << endl;
...
int CoreRunEvaluate01::ScheduleASubscription(Subscription* _PS, vector<Message*>& _ScheduledMessages, bool _ClearScheduledMessage)
{
  Message* Run = 0;
  CommandLine* PCL = 0;
  Core* PCore = 0;
...
int CoreRunEvaluate01::ProcessASubscribedPayload(string _Publisher_LN, unsigned int _Index, Subscription* _PS, vector<Message*>& _ScheduledMessages, bool _ClearScheduledMessage)
{
  Core* PCore = 0;
  string Offset1 = "                              ";
...
int CoreRunEvaluate01::ProcessTXTFile(string _Publisher_LN, unsigned int _Index, Subscription* _PS, string _FileName)
{
  ...
  if (Temp == "Service_Offer")
  {
    ...
    // Generate the SCN
    SCN1 = NameGenerator::GetInstance().GenerateFromMessage(PublishAcceptance);
  ...
  if (Temp1 == "PGCSFile")
  {
    ...
    string Command1 =
        "sudo /usr/bin/python coapclient.py -o POST -p coap://[aaaa::212:4b00:7ba:7607]:5683/dev/chan -P \"ch=" + Value + "\" \& sudo /usr/bin/python coapclient.py -o POST -p coap://[aaaa::212:4b00:6e3:660c]:5683/com/chan -P \"ch=" + Value + "\"";
