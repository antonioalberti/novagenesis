...
CoreRunInitialize01::Run (Message *_ReceivedMessage, CommandLine *_PCL, vector<Message *> &ScheduledMessages, Message *&InlineResponseMessage)
{
  ...
  // Creating a new message
  PB->PP->NewMessage (GetTime () + PCore->DelayBeforeFirstPeriodic, 1, false, RunPeriodic);
  ...
  while (F3.getline (Line, sizeof (Line), '\n'))
	{
	  ...
	  if (Parameter == "DelayBeforeFirstPeriodic")
		{
		  Temp = PB->StringToDouble (Value);

		  PCore->DelayBeforeFirstPeriodic = Temp;

		  PB->S << Offset << "DelayBeforeFirstPeriodic is " << Temp << endl;
