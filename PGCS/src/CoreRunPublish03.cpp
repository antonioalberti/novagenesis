		  ...
		  if (PeerData.size () == 2)
			{
			  ...
			  PB->PP->NewMessage (GetTime (), 1, true, "Not_Necessary", PeerData.at (1), "Not_Necessary", PB
				  ->GetPath (), Publish);
			  ...
			  if (PayloadSize > 0)
				{
				  ...
				  PCore->Debug << "Publishing to the peer " << PeerData.at (0) << " the file " << PeerData.at (1)
							   << ". The file hash is " << PayloadHash << endl;
