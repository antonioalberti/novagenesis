...
IRMsgCl02::IRMsgCl02(string _LN, Block* _PB, MessageBuilder* _PMB)
    : Action(_LN, _PB, _PMB)
{
}
...
int IRMsgCl02::Run(Message* _ReceivedMessage, CommandLine* _PCL, vector<Message*>& ScheduledMessages, Message*& InlineResponseMessage)
{
  int Status = ERROR;
  unsigned int NA = 0;
            ...
            if (PB->State == "operational")
            {
              ...
              if (PIR->HTSTuples.size() > 0)
              {
                ...
                for (unsigned int i = 0; i < PIR->HTSTuples.size(); i++)
                {
                  ...
                  if (PMB->NewConnectionLessCommandLine("0.1", &Limiters, &Sources, &Destinations, Forward, PCL) == OK)
                  {
