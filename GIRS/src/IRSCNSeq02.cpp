...
IRSCNSeq02::IRSCNSeq02(string _LN, Block* _PB, MessageBuilder* _PMB)
    : Action(_LN, _PB, _PMB)
{
}
...
int IRSCNSeq02::Run(Message* _ReceivedMessage, CommandLine* _PCL, vector<Message*>& ScheduledMessages, Message*& InlineResponseMessage)
{
  int Status = ERROR;
  string ReceivedSCN;
  string NewSCN;
      ...
      if (ReceivedSCN != "")
      {
        ...
        if (ScheduledMessages.size() > 0)
        {
          ...
          for (unsigned int i = 0; i < ScheduledMessages.size(); i++)
          {
            ...
            NewSCN = NameGenerator::GetInstance().GenerateFromMessage(PM);
