...
IRRunPeriodic01::IRRunPeriodic01(string _LN, Block* _PB, MessageBuilder* _PMB)
    : Action(_LN, _PB, _PMB)
{
  PGCSPID = "unknown";
...
int IRRunPeriodic01::Run(Message* _ReceivedMessage, CommandLine* _PCL, vector<Message*>& ScheduledMessages, Message*& InlineResponseMessage)
{
  int Status = ERROR;
  string Offset = "                    ";
  IR* PIR = 0;
...
int IRRunPeriodic01::Operational()
{
  ...
  if (GoToOperational1 == true && GoToOperational2 == true && PB->State != "operational")
  {
    ...
    PMB->NewGetCommandLine("0.1", 19, PIR->PSSTuples[0]->Values[2], IPCUpdate, PCL);

    PMB->NewGetCommandLine("0.1", 19, PIR->HTSTuples[0]->Values[2], IPCUpdate, PCL);
