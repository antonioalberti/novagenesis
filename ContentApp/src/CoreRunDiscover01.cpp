...
#ifdef DEBUG
					  PB->S << Offset
							<< "(Sending a discovery message to the PGCS process on the same operating system)" << endl;
#endif
						  ...
						  if (Pair.size () > 0)
							{
							  ...
							  PB->S << Offset << "(Checking if this binding already exists on the discovery message)"
									<< endl;
#endif
