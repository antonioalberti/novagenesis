...
int CoreRunContentPublish01::Run(Message* _ReceivedMessage, CommandLine* _PCL, vector<Message*>& ScheduledMessages, Message*& InlineResponseMessage)
{
  int Status = ERROR;
  string Offset = "                    ";
  Core* PCore = 0;
    ...
    for (int y = 0; y < FileNamesInThePath.size(); y++)
    {
      ...
      if (Extension != "" && Extension != "ini" && Extension != "txt" && Extension != "dat" && Extension != "sh" && Extension != "py" && Extension != "out")
      {
        ...
        if (PublicationStatus == true)
        {
          ...
          PB->S << Offset << "(Messages window = " << Allowed << ")" << endl
                << endl;
#endif
