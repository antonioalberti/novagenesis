  ...
  if (PCore->PSTuples.size() > 0)
  {
    if (PCore->NextPeerEvaluationTime < Time)
    {
...
#ifdef DEBUG
      PB->S << endl
            << Offset1 << "(Check for new peer server application tuples)" << endl;
#endif
      ...
      if (PB->PP->DiscoverHomonymsEntitiesIDsFromLN(2, "Repository", Servers, PB) == OK &&
          ...
          PB->PP->DiscoverHomonymsEntitiesTuplesFromProcessAndBlockLegibleNames("ContentApp", "Core", Apps, PB) == OK)
      {
#ifdef DEBUG
        PB->S << endl
              << Offset1 << "(Servers vector size = " << Servers->size() << ")" << endl;
                ...
                for (unsigned int k = 0; k < Apps.size(); k++)
                {
                  ...
                  if (Servers->at(i) == Apps[k]->Values[2])
                  {
                    ...
                    if (StoreFlag == true)
                    {
                      ...
                      bool HasCat5 = (PB->PP->GetHTBindingValues(5, Servers->at(i), Cat5BIDs) == OK && Cat5BIDs->size() > 0);
                      delete Cat5BIDs;
      ...
      PB->S << endl
            << Offset1 << "(Check for new peer client application tuples)" << endl;
...
#ifdef DEBUG
        PB->S << endl
              << Offset1 << "(Clients vector size = " << Clients->size() << ")" << endl;
        PB->S << Offset1 << "(Subject vector size = " << Subject->size() << ")" << endl;
        PB->S << Offset1 << "(App tuples vector size = " << Apps.size() << ")" << endl;
              ...
              if (Run != 0)
              {
                if (PCore->GetFileContentHash(PS->FileName, PayloadHash) == OK)
                {
                  ...
                  if (PS->Key == PayloadHash)
                  {
                    ...
                    AcceptedPayload =
                        "Alberti_HUID Accepted " + PB->PP->GetSelfCertifyingName() + " " + PS->Publisher.Values[2];
