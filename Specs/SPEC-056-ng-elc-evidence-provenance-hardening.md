...
Commits `a571e7e`, `dbf1241` and `c1f5ad8` wire the local lifecycle to invoke `verify_bundle()` after sealing, add the R02 deterministic five-file workload and add canonical R06 provenance/build-linkage capture with fail-closed executable drift/missing-manifest checks. Remote schema-v1 behaviour remains preserved. Astra review `Specs/RESULTS-SPEC-056/astra-r06-review-20260914.md` returned `HOLD / NO-GO`: linkage completeness, executable collision handling, stable dirty snapshots, secret-safe capture and launch identity still require remediation before a candidate trial. A clean candidate trial with a production build manifest, complete ownership/cleanup evidence and local acceptance remain open under this SPEC.
