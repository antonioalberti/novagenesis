...
Commits `a571e7e`, `dbf1241` and `c1f5ad8` wire the local lifecycle to invoke `verify_bundle()` after sealing, add the R02 deterministic five-file workload and add canonical R06 provenance/build-linkage capture with fail-closed executable drift/missing-manifest checks. Remote schema-v1 behaviour remains preserved. Astra reviews through `Specs/RESULTS-SPEC-056/astra-r10-review-20260914.md` returned `HOLD / NO-GO`. R08/R09/R10 resolve or improve immutable launch binding, staged-index/linkage checks and lifecycle controls, but protected-input fail-closed publication, descendant-safe rollback, semantic build identity and retained local/remote acceptance evidence remain open. A clean candidate trial with a production build manifest, complete ownership/cleanup evidence and local acceptance remain open under this SPEC.
