...
Commits `a571e7e`, `dbf1241` and `c1f5ad8` wire the local lifecycle to invoke `verify_bundle()` after sealing, add the R02 deterministic five-file workload and add canonical R06 provenance/build-linkage capture with fail-closed executable drift/missing-manifest checks. Remote schema-v1 behaviour remains preserved. Astra reviews `Specs/RESULTS-SPEC-056/astra-r06-review-20260914.md`, `Specs/RESULTS-SPEC-056/astra-r07-review-20260914.md` and `Specs/RESULTS-SPEC-056/astra-r08-review-20260914.md` returned `HOLD / NO-GO`. R08 resolves immutable launch binding and staged-index drift at source level, but build identity semantics, quoted/bundle secret safety, descendant rollback and lifecycle/remote test coverage remain open. A clean candidate trial with a production build manifest, complete ownership/cleanup evidence and local acceptance remain open under this SPEC.
