...
Commits `a571e7e`, `dbf1241` and `c1f5ad8` wire the local lifecycle to invoke `verify_bundle()` after sealing, add the R02 deterministic five-file workload and add canonical R06 provenance/build-linkage capture with fail-closed executable drift/missing-manifest checks. Remote schema-v1 behaviour remains preserved. Astra reviews `Specs/RESULTS-SPEC-056/astra-r06-review-20260914.md` and `Specs/RESULTS-SPEC-056/astra-r07-review-20260914.md` both returned `HOLD / NO-GO`. R07 closes basename-collision handling but still leaves launch binding, complete build/index identity, secret-safe capture, process registration and lifecycle-test gaps. A clean candidate trial with a production build manifest, complete ownership/cleanup evidence and local acceptance remain open under this SPEC.
