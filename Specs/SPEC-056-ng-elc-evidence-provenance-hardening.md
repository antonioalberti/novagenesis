...
Commits `a571e7e`, `dbf1241` and `c1f5ad8` wire the local lifecycle to invoke `verify_bundle()` after sealing, add the R02 deterministic five-file workload and add canonical R06 provenance/build-linkage capture with fail-closed executable drift/missing-manifest checks. Remote schema-v1 behaviour remains preserved. Astra reviews through `Specs/RESULTS-SPEC-056/astra-r11-review-20260914.md` returned `HOLD / NO-GO`. R08–R11 resolve or improve immutable launch binding, staged-index/linkage checks, blocker latching and lifecycle controls, but unsafe-file publication after quarantine failure, late/reparented descendant ownership, recipe-to-build/runtime linkage, argv secret coverage and retained local/remote acceptance evidence remain open. A clean candidate trial with a production build manifest, complete ownership/cleanup evidence and local acceptance remain open under this SPEC.
