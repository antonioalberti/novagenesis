...
E="Specs/RESULTS-SPEC-054/g2-evidence-20260913/smoke"
mkdir -p "$E"
...
printf '%s\n' 'phase=PGCS-Source'
timeout --signal=TERM --kill-after=10s 180 bash Scripts/AlpineVMs/run_PGCS_on_Source_VM.sh >"$E/01-pgcs-source.log" 2>&1 & p1=$!
PIDS="$PIDS $p1"
...
if [ "$PHASE" = "PGCS_SOURCE_READY" ]; then
    ...
    timeout --signal=TERM --kill-after=10s 180 bash Scripts/AlpineVMs/run_PGCS_on_Repo_VM.sh >"$E/02-pgcs-repository.log" 2>&1 & p2=$!
    PIDS="$PIDS $p2"
    for _ in $(seq 1 90); do
...
if [ "$PHASE" = "PGCS_BOTH_READY" ]; then
    printf '%s\n' 'phase=NRNCS'
    timeout --signal=TERM --kill-after=10s 180 bash Scripts/AlpineVMs/run_NRNCS_on_Source_VM.sh >"$E/03-nrncs.log" 2>&1 & p3=$!
    PIDS="$PIDS $p3"
...
if [ "$PHASE" = "NRNCS_READY" ]; then
    printf '%s\n' 'phase=Repository'
    timeout --signal=TERM --kill-after=10s 180 bash Scripts/AlpineVMs/run_Repository_on_Repo_VM.sh >"$E/04-repository.log" 2>&1 & p4=$!
    PIDS="$PIDS $p4"
    for _ in $(seq 1 90); do
...
if [ "$PHASE" = "REPOSITORY_STARTED" ]; then
    printf '%s\n' 'phase=Source'
    timeout --signal=TERM --kill-after=10s 180 bash Scripts/AlpineVMs/run_Source_on_Source_VM.sh 1 100 100 >"$E/05-source.log" 2>&1 & p5=$!
    PIDS="$PIDS $p5"
    for _ in $(seq 1 120); do
